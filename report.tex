...
\emph{Jingle} (define in XEP-0166\cite{jingle166} and XEP-0167\cite{jingle167})
...
protocols often used with SIP, such as ICE\cite{ice}, ZRTP\cite{zrtp}, DTLS-SRTP\cite{dtls-srtp}, and
...
Google Summer of Code program, on
a recording
application which uses the fake participant approach--
 \emph{Jirecon}\footnote{https://github.com/jitsi/jirecon}.



\section{The WebRTC transport layer XXX rename (s/WebRTC//?)}
...
\begin{wrapfigure}{R}{4cm}
   \centering
        ...
        \caption{The chain of \emph{PacketTransformer}s in \lj. The shaded elements are new additions added while implementing the recording functionality.}
   \label{lj-pt}
\end{wrapfigure}
...
\label{ulpfec}
XXX stupid idea with re-writing sequence numbers

XXX why adding FEC arbitrary to the stream makes it impossible to decode correctly

XXX what webrtc does and how we handle it by just removing my stupid code
...
triggered by RTCP NACK

chrome just retransmits 

we handle it in the JB for video 
\subsection{RTCP compound packets}
not much to say, packets are concattenated, we split them...in a \emph{PacketTransformer}, of course..
...
\smallskip
We found a library with a simple API that would allow us to write \emph{webm} files
very easily, so we decided to ignore \emph{ivf}, postpone the potential
