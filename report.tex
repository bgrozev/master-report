...
it encodes the packet at a lower quality (and lower bitrate), and attaches this
...
needed) automatically. Also, the audio mixer serves as a capture device, and
...
started to think about synchronizing the recorder audio and video. In order to
...
We decided that we needed to give up the idea of using the \lj\ audio mixer and
...
many of the tasks which we require.

We adapted \emph{RecorderRtpImpl} so that it could handle streams with
different formats. The existing code in \lj\ supports recording only in
\emph{mp3} format. We decided to use this, for the time being, because it is
relatively easy to later transcode it to whatever format is needed.

For an audio stream, we use FMJ to decode (to a PCM format), and
then encode it to \emph{mp3} and save it in an \emph{mp3} format.
...
samples played one after the other. This means that if some samples are missing
...
Repairing such a gap of, for example, 10 minutes would require a relatively
large amount of computation. In such cases it would be better to re-start the
recording for the stream in a new file, giving it it's own timestamp in the
...
section \label{synchronization} for the details)


\subsection{Overview}
Initially we started with recording a mix of all audio streams, because that
was the easiest solution. We later change to recording streams individually.
...
\begin{figure}[h]
    ...
    \caption{The chain used to reencode audio for the purposes of recording.}
    \label{audio-rec}
\end{figure}

This solution involves re-encoding and that makes it suboptimal -- it is both
more computationally expensive than necessary and reduces the sound quality of the recording.
A better solution would be, whenever possible, to record in a contained format
that does not require encoding (as we do for video). For \emph{Opus}, a viable solution would be
to use the ogg/opus format and we plan to implement, see section \ref{ogg-opus}.
