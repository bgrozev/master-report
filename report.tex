...
%XXX add dtls to bib
to exchange key material, which is then used to generate session keys for an
...
XXX This section is about transporting media in webrtc. Uses ICE to establish a
...
\subsection{RTCP compunt packets}
...
\item merge
\end{itemize}
a. and v. processing are independent.
ffmpeg is heavily used (command line, not API), a fixed frame rate used for output, all webm videos are converted to MJPEG, result is endoded
    decode all,
    do sections,
    concat sections,
    encode result
